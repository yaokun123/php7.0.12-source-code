...
struct _zend_mm_heap {
	...
	size_t             peak;                    /* peak memory usage 历史内存使用的峰值*/
#endif
	...
	size_t             real_size;               /* current size of allocated pages 当前已经分配page*/
#endif
#if ZEND_MM_STAT
	size_t             real_peak;               /* peak size of allocated pages 分配page的峰值*/
#endif
#if ZEND_MM_LIMIT
	size_t             limit;                   /* memory limit 内存限制*/
	int                overflow;                /* memory overflow flag 内存益处标志*/
#endif
	...
	double             avg_chunks_count;		/* average number of chunks allocated per request chunk使用均值，每次请求结束后会根据peak_chunks_count重新计算：(avg_chunks_count+peak_chunks_count)/2.0*/
#if ZEND_MM_CUSTOM
	union {
		struct {
			void      *(*_malloc)(size_t);
...
struct _zend_mm_chunk {
	...
	int                free_pages;				/* number of free pages 当前chunk的剩余page数*/
	...
	zend_mm_page_info  map[ZEND_MM_PAGES];      /* 2 KB = 512 * 4 各page的信息：当前page使用类型(用于large分配还是small)、占用的page数等*/
};
...
struct _zend_mm_free_slot {/* 按固定大小切好的small内存槽 */
...
{//size为申请内存的大小，alignment为内存对齐值，一般为ZEND_MM_CHUNK_SIZE
	void *ptr = zend_mm_mmap(size);//向系统申请size大小的内存
	...
	if (ptr == NULL) {
...
#endif
		return ptr;
	} else {//申请的内存不是按照alignment对齐的，注意这里的alignment并不是系统的字节对齐值
		size_t offset;

		/* chunk has to be aligned */
		zend_mm_munmap(ptr, size);//将申请的内存释放掉重新申请
		ptr = zend_mm_mmap(size + alignment - REAL_PAGE_SIZE);//重新申请一块内存，这里会多申请一块内存，用于截取到alignment的整数倍，可以忽略REAL_PAGE_SIZE
#ifdef _WIN32
		...
		return ptr;
#else
		offset = ZEND_MM_ALIGNED_OFFSET(ptr, alignment);//offset为ptr距离上一个alignment对齐内存位置的大小，注意不能往前移，因为前面的内存都是分配了的
		if (offset != 0) {
			offset = alignment - offset;
			zend_mm_munmap(ptr, offset);
			ptr = (char*)ptr + offset; //偏移ptr，对齐到alignment
			alignment -= offset;
		}
...
#endif
{
	...
	while (1) {//从第一个chunk开始查找可用page
		if (UNEXPECTED(chunk->free_pages < pages_count)) {//当前chunk剩余page总数已不够
			goto not_found;
#if 0
...
found:
	/* mark run as allocated */ //找到可用page，page编号为page_num至(page_num + pages_count)
	...
	if (page_num == chunk->free_tail) {//map为两个值的组合值，首先表示当前page属于哪种类型，其次表示包含的page页数
		chunk->free_tail = page_num + pages_count;
	}
...
{
	int pages_count = (int)ZEND_MM_SIZE_TO_NUM(size, ZEND_MM_PAGE_SIZE);//根据size大小计算需要分配多少个page
#if ZEND_DEBUG
	void *ptr = zend_mm_alloc_pages(heap, pages_count, size ZEND_FILE_LINE_RELAY_CC ZEND_FILE_LINE_ORIG_RELAY_CC);//分配pages_count个page
#else
...
static void *zend_mm_alloc_huge(zend_mm_heap *heap, size_t size ZEND_FILE_LINE_DC ZEND_FILE_LINE_ORIG_DC)
{
...
#if ZEND_MM_LIMIT   //如果有内存使用限制则check是否已达上限，达到的话进行zend_mm_gc清理后再检查(此过程不再展开分析)
	...
	if (UNEXPECTED(ptr == NULL)) {//清理后再尝试分配一次
		/* insufficient memory */
		if (zend_mm_gc(heap) &&
...
#if ZEND_DEBUG
	zend_mm_add_huge_block(heap, ptr, new_size, size ZEND_FILE_LINE_RELAY_CC ZEND_FILE_LINE_ORIG_RELAY_CC);//将申请的内存通过zend_mm_huge_list插入到链表中,heap->huge_list指向的实际是zend_mm_huge_list
#else
...
{
	zend_mm_chunk *chunk = (zend_mm_chunk*)zend_mm_chunk_alloc_int(ZEND_MM_CHUNK_SIZE, ZEND_MM_CHUNK_SIZE);//向系统申请2M大小的chunk
	zend_mm_heap *heap;
	...
	chunk->next = chunk;
	chunk->prev = chunk;
	chunk->free_pages = ZEND_MM_PAGES - ZEND_MM_FIRST_PAGE;//剩余可用page数（512-1=511）减去的1用于保存chunk中page结构
	chunk->free_tail = ZEND_MM_FIRST_PAGE;
	chunk->num = 0;
