...
/* build with dtrace support */
...
#else
	zend_compile_file = compile_file;//提供编译与执行入口
	zend_execute_ex = execute_ex;//提供编译与执行入口
	...
	GLOBAL_FUNCTION_TABLE = (HashTable *) malloc(sizeof(HashTable));//初始化CG、EG
	GLOBAL_CLASS_TABLE = (HashTable *) malloc(sizeof(HashTable));//初始化CG、EG
	GLOBAL_AUTO_GLOBALS_TABLE = (HashTable *) malloc(sizeof(HashTable));//初始化CG、EG
	GLOBAL_CONSTANTS_TABLE = (HashTable *) malloc(sizeof(HashTable));//初始化CG、EG
	...
	*executor_globals->zend_constants = *GLOBAL_CONSTANTS_TABLE;
#else
	ini_scanner_globals_ctor(&ini_scanner_globals);//初始化ini_scanner_globals
	php_scanner_globals_ctor(&language_scanner_globals);//初始化language_scanner_globals
	...
	zend_register_auto_global(zend_string_init("GLOBALS", sizeof("GLOBALS") - 1, 1), 1, php_auto_globals_create_globals);//++zend_register_standard_constants++。注册常量，比如E_ERROR、E_WARNING、E_NOTICE、E_CORE_ERROR等。
