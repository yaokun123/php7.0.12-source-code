...
<a href=\"http://php.net/install.windows\">the manual page</a>.</p>\n");
	...
	request = fpm_init_request(fcgi_fd);    //fcgi_fd就是监听的套接字，request是经过包装后的套接字

	zend_first_try {
		while (EXPECTED(fcgi_accept_request(request) >= 0)) {//1、等待请求：worker进程阻塞在fcgi_accept_request()等待请求
			 ...
			 *            get path_translated */
			if (UNEXPECTED(php_request_startup() == FAILURE)) {//请求开始
			...
			php_execute_script(&file_handle);   //编译、执行PHP脚本
...
fastcgi_request_done:
			...
			php_request_shutdown((void *) 0);   //请求结束

			requests++;                                //记录每个worker进程处理的请求数
			if (UNEXPECTED(max_requests && (requests == max_requests))) {//如果请求数达到配置的数量，该worker进程就会结束，master进程监听到worker进程结束后会重启一个新的worker进程
				fcgi_finish_request(request, 1);
				break;
			}
			/* end of fastcgi loop */
