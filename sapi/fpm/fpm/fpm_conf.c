...
int fpm_conf_init_main(int test_conf, int force_daemon) /* {{{ */
{
	...
	if (fpm_globals.config == NULL) {	//如果fpm配置文件不存在，使用默认的配置文件，一般不会走到这里
		char *tmp;

		if (fpm_globals.prefix == NULL) {	//如果没有设置前缀
			// /Users/yaokun/php-private/php-source-code/php7.0.12/etc/php-fpm.conf
			spprintf(&tmp, 0, "%s/php-fpm.conf", PHP_SYSCONFDIR);
		} else {
			// fpm_globals.prefix + '/etc/php-fpm.conf'
			spprintf(&tmp, 0, "%s/etc/php-fpm.conf", fpm_globals.prefix);
		}

		if (!tmp) {		//配置文件没有找到，返回错误
	...
	ret = fpm_conf_load_ini_file(fpm_globals.config);	//解析php-fpm.conf配置文件

	if (0 > ret) {		//配置文件加载失败，返回错误
		zlog(ZLOG_ERROR, "failed to load configuration file '%s'", fpm_globals.config);
		return -1;
	}

	if (0 > fpm_conf_post_process(force_daemon)) {	//分配全局变量：fpm_worker_pool_s
