...
static void fpm_pctl_perform_idle_server_maintenance(struct timeval *now) /* {{{ */
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {//一般只有一个进程池
		...
		if (wp->config->pm == PM_STYLE_ONDEMAND) { //ondemand模式
		...
		if (wp->config->pm != PM_STYLE_DYNAMIC) continue;	//dynamic
...
void fpm_pctl_perform_idle_server_maintenance_heartbeat(struct fpm_event_s *ev, short which, void *arg) /* {{{ */
{
	...
	if (which == FPM_EV_TIMEOUT) {
		...
		if (fpm_pctl_can_spawn_children()) {
			fpm_pctl_perform_idle_server_maintenance(&now);//定时检查各worker pool的worker进程数
