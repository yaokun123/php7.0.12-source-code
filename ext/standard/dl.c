...
PHPAPI int php_load_extension(char *filename, int type, int start_now)
{
	...
	/* load dynamic symbol */
	handle = DL_LOAD(libpath);  //1、调用dlopen打开指定的动态连接库文件：xx.so
	if (!handle) {
#if PHP_WIN32
	...
	get_module = (zend_module_entry *(*)(void)) DL_FETCH_SYMBOL(handle, "get_module");  //2、调用dlsym获取get_module的函数指针
	...
	module_entry = get_module();    //3、调用扩展的get_module()函数，获取扩展的zend_module_entry结构。
	                                // get_module()是每个扩展都必须提供的一个接口，用于返回扩展zend_module_entry结构的地址
	if (module_entry->zend_api != ZEND_MODULE_API_NO) { //4、检查扩展使用的zend api是否与当前php版本一致
	...
	module_entry->module_number = zend_next_free_module();  //5、为扩展编号
	module_entry->handle = handle;

	if ((module_entry = zend_register_module_ex(module_entry)) == NULL) {//6、注册扩展，将扩展添加到module_registry中，这是一个全局HashTable，用于全部扩展的zend_module_entry结构
		DL_UNLOAD(handle);
		return FAILURE;
	}

	if ((type == MODULE_TEMPORARY || start_now) && zend_startup_module_ex(module_entry) == FAILURE) {//7、调用zend_module_entry.module_startup_func()
		DL_UNLOAD(handle);
		return FAILURE;
	}
