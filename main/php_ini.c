...
/* {{{ php_load_php_extension_cb：HAVE_LIBDL这个宏根据dlopen()函数是否存在设置的
 */
...
/* {{{ php_ini_register_extensions: 动态库(.so/dll)就是在这个函数中完成的注册
 *
 * &extension_lists.functions 是一个链表（zend_llist）保存着根据php.ini中定义的extension=xxx.so取到的全部扩展名称
 * 其中engine是zend扩展，functions为php扩展
 */
void php_ini_register_extensions(void)
{
	zend_llist_apply(&extension_lists.engine, php_load_zend_extension_cb);      //注册zend扩展
	zend_llist_apply(&extension_lists.functions, php_load_php_extension_cb);    //注册php扩展
