...
# endif
	...
	zend_try {
		...
		if (PG(max_input_time) == -1) {
			...
			zend_set_timeout(PG(max_input_time), 1);//设置超时时间
		}
...
int php_module_startup(sapi_module_struct *sf, zend_module_entry *additional_modules, uint num_additional_modules)
{
	...
	module_shutdown = 0;
	module_startup = 1;
	sapi_initialize_empty_request();    //这个函数主要为前面定义的SG宏中的成员变量进行初始化。


	//函数的前半部分主要还是对SG宏的成员变量进行初始化。
	// 后半部分先是调用了sapi_module_struct内部实现的activate函数，
	// 又调用了input_filter_init函数，但是在CLI模式并没有实现这两个函数，只是返回了NULL。
	sapi_activate();
	...
	php_output_startup();   //初始化输出相关结构体，OG宏
...
#endif
#else
	php_startup_ticks();//对PG宏的成员变量tick_functions进行初始化
#endif
	gc_globals_ctor();//垃圾回收相关
	...
	le_index_ptr = zend_register_list_destructors_ex(NULL, NULL, "index pointer", 0);//初始化析构函数
	...
	/* this will read in php.ini, set up the configuration parameters,
	   ...
	   to be loaded later */
	if (php_init_config() == FAILURE) {//初始化配置文件php.ini，并通过zend_parse_ini_file解析
		return FAILURE;
	}
	...
	zend_register_standard_ini_entries();//初始化ini相关的变量
	...
	php_startup_auto_globals();//注册我们熟知的全局变量$_GET、$_POST、$_COOKIE等等
	zend_set_utility_values(&zuv);
	php_startup_sapi_content_types();//初始化针对不同内容类型的处理函数
	 ...
	 */
	php_ini_register_extensions();  //根据php.ini注册扩展
